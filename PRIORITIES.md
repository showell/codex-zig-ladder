...
- **PR 77** -- the heap unification, sent 2026-08-23 on `8cb8a0e4`
  ...
  Filed, not landed.
- **PR 81** -- finding 33's fix, the zig plug's tail-call loop, SENT
  2026-08-24. **Stacked on PR 77**: the branch is cut from
  `zig-plug-heap-unification` `8cb8a0e4`, so its first 21 commits are 77's
  and only the last four (`6cd40143`, `07495229`, `912daac7`,
  `64d7db8e`) are this PR's; the true three-dot diff is two files.
  Stacked deliberately rather than rebased onto master, because the 14/14
  sweep verified THAT tree and a rebase would have made the branch a
  tree nothing had measured. Ladder tag `tail-calls`. Open, not landed,
  and it lands only after 77 does.
