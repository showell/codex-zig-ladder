...
def main():
    ...
    same_build = same_diag = same_run = 0
    built = ran = 0
    problems = []
    with tempfile.TemporaryDirectory(prefix='postlude-') as td:
        ...
        for n, src in enumerate(srcs, 1):
            ...
            after_src = work / (src.stem + '.postlude.zig')
            after_src.write_text(to_postlude(text, banner))

            ok_a, diag_a, run_a = build_and_run(src, work, 'a')
            ok_b, diag_b, run_b = build_and_run(after_src, work, 'b')
            after_src.unlink()
