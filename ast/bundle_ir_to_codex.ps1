# Bundle the text-milestone subject: the lower subject plus the compiler's
# own codex-text emitter. Output: text-subject.codex next to this script.
#
# CodexEmitter cites one chapter, Build Settings, which is already here.
# It is Damian's production emitter, untouched by this branch -- the point
# of the milestone is that the plug can carry it, not that we can write a
# serializer.
#
# Lowering cites only Build Settings and Phase Allocator, both already here
# for the check subject, so the delta really is the two chapters. It binary
# searches the type bindings (lookup-type-bsearch), which is why the harness
# sorts them with sort-bindings from LoweringTypes rather than handing over
# the checker's output as it comes.
#
...
param(
    [string]$Harness = 'IrToCodexHarness.codex',
    [string]$OutName = 'ir_to_codex-subject.codex'
)
...
#
# Prose is 80 KB of that and none of it is executable: a line indented by
# exactly one space is prose, two marks a definition, and bodies are
# indented further, so the rule is unambiguous. Stripping it changes no
# behaviour. It does NOT touch the emitted output either, because the
# chapter being emitted is the harness's fib subject, not the compiler
# chapters carrying this prose.
#
# Only this bundle strips. The other milestones fit without it and their
# banked truth stays valid.
$stripped = [System.Collections.Generic.List[string]]::new()
foreach ($l in $lines) { if (-not ($l -match '^ [^ ]')) { $stripped.Add($l) } }
$lines = $stripped
...
$preLines = $prePlain
Bundle-PlugSource -PreLines $preLines -Lines $lines -BundleSrc (Join-Path $here $OutName) -PlugName 'ir_to_codex-subject'
