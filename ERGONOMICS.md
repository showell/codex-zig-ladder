...
arm, `tier_run.py`, `src/arithcycle.sh`, `bare_expected.py`. `compile_blob`
...
**PowerShell was never the barrier either.** `src/oracle_lib.sh:159`
...
- `kill` on `verify_emitter.sh` left `src/allcycles.sh` running. It went
...
**A second, smaller thing in the same file.** `src/ringplug_build.sh:36`
...
**What prompted it.** `src/*Harness.codex` is generated by
`src/gen_*_harness.py` and gitignored in place. On 2026-08-26 I edited
`src/CodexIrHarness.codex` directly to add finding 49's error gate --
...
`src/oracle_lib.sh` and the bundlers all read these paths -- so it wants
